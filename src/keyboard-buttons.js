// Screens
module.exports = {
    homeBtn: `🌐 Главное меню`,
    home: {
        form: `📊 Опрос`,
        calendar: `📆 Календарь событий`,
        smallGroups: `🏠 Малые Группы`,
        serv: `🙌🏻 Служения`,
        learnMore: `📚 Узнать больше`,
        mainSteps: `👣 ШАГИ`,
        pray: `🙏🏻 Молитвенные нужды`,
        faq: `❔ Вопросы`,
        materialsForSG: `💻 Материалы для МГ`,
    },
    calendar: {
        game: `🎲 Вечер настольных игр`,
        pray: `🙏🏻 Молодежная молитва`,
        turnout: `🏆 Боулинг`,
        teaWithPastor: '☕️ Чай с Пастором',
        tokshow: '🎤 ТОК-ШОУ',
        stepsInSundays: `👣 ШАГИ`,
        domashkaParty: '🏄 Домашка-Party',
        goHome: `🌐 Главное меню`,
    },
    smallGroups: {
        sgMember: `🔸Хочу стать участником Малой Группы`,
        sgLeader: `🔸Хочу стать ведущим Малой Группы`,
        sgMap: `🗺 Карта Малых Групп`,
        goHome: `🌐 Главное меню`,
    },
    serv: {
        servAll: `📜 Список служений`,
        servHire: `👀 Мы ищем`,
        servunknown: `🤷🏼 Хочу служить, но не знаю где`,
        goHome: `🌐 Главное меню`,
    }, 
    servAll: {
        servMyGeneration: `💪🏼 Моё Поколение`,
        servWorship: `🎹 Группа Прославления`,
        servInfo: `ℹ️ Стойка Информации`,
        servCafe: `☕️ Служение в кафе`,
        servAsher: `😎 Группа Порядка`,
        servCommunion: `🍷 Служение Причастия`,
        servChild: `👶🏼 Детское служение`,
        servComunication: `🥳 Команда цели Общение`,
        servMeeting: `🤝 Знакомство с Церковью`,
        servSocialWeb: `📱 Медиа отдел`,
        servHello: `🤚🏻 Служение приветствия`,
        servTeenagers: `🛹 Подростковое служение`,
        servPray: `🙏🏻 Молитвенная команда`,
        servTheatre: `🎭 Театральное служение`,
        servFamily: `👨‍👩‍👧‍👦 Семейное служение`,
        servDecoration: `🌿 Оформление зала`,
        servYouthWorship: `🎸 Молодежное прославление`,
        servOutreach: `🏛 Уличное благовестие`,
        servFonds: `Фонды и др.служения`,
        backToServ: `Назад к '🙌🏻 Служения'`,
    },
    learnMore: {
        aboutChurch: '✝️ О Церкви',
        spiritUp: '🆙 Духовный рост',
        sozo: '🆗 Душепопечение',
        familyServ: '✅ Семейное служение',
    }, 
    onePlusOne: {
        findMentor: `🔎 Хочу найти наставника`,
        becomeMentor: `🎓 Хочу стать наставником`,
        backToTeaching: `Назад к '📚 Обучение'`
    },
    // notes: {},
    // pray: {},
    // faq: {},
    materialsForSG: {
        msgHelp: `📔 Помощь`,
        msgNotes: `🗓 Конспекты`,
        msgWorship: `🎶 Прославление`,
        msgBooks: `📖 Книги`,
        msgVideo: `🟥 Видео-ресурсы`,
        msgMeetings: `🕖 Встречи`,
        goHome: `🌐 Главное меню`,
    },
    // Inline keyboards

    sgMap_inline: 'Карта Малых Групп',

    // Calendar inline_keyboard
    apple: 'Apple',
    google: 'Google',
    // First Item ============================ 🎲 ВЕЧЕР НАСТОЛЬНЫХ ИГР =============================
    appleItemFirst: 'https://calndr.link/e/DDgvtGdaKH'+'?s=apple',
    googleItemFirst: 'https://calndr.link/e/DDgvtGdaKH'+'?s=google',
    // Second Item =========================== 🙏🏻 МОЛОДЕЖНАЯ МОЛИТВА ===============================
    appleItemSecond: 'https://calndr.link/e/N0LXaPrTun'+'?s=apple',
    googleItemSecond: 'https://calndr.link/e/N0LXaPrTun'+'?s=google',
    // Third Item ============================ ДомашкаПати =========================================
    appleItemThird: 'https://calndr.link/e/HQLXeygcTM'+'?s=apple',
    googleItemThird: 'https://calndr.link/e/HQLXeygcTM'+'?s=google',
    // Fourth Item =========================== ТОК-ШОУ =============================================
    appleItemFourth: 'https://calndr.link/e/9xmZkdOjHo'+'?s=apple',
    googleItemFourth: 'https://calndr.link/e/9xmZkdOjHo'+'?s=google',
    // Fifth Item
    appleItemFifth: ''+'?s=apple',
    googleItemFifth: ''+'?s=google',

    // Respond inline_keyboard
    respondBtn: 'Написать',
    respondForSG: 't.me/nesilchuk',
    respondUrl: 'https://t.me/mbvadmin',

    messToKarina: 't.me/karina_dmitrienko',

    // Vitalik 
    signUpBtn: 'Записаться',
    signUpUrl: 't.me/vitalik_golikov',

    // Serv All
    writeLeader: `Написать лидеру служения`,
}